...
public class HomeScreen {
    ...
    private static final int BTN_GAP       = 8;    // espacio entre tarjeta y botón de desconexión
    ...
    public void init(PanelUI panel, int screenWidth, int screenHeight, PanelUI.DeviceInfo device) {
        ...
        // Botón de desconexión: justo debajo de la tarjeta + separación
    ...
    public void render(GuiGraphics gui, int mouseX, int mouseY, Font font, int width, int height) {
        ...
        gui.drawString(font, "INICIO",
                (int)(x / scale),                             (int)(22 / scale), 0xFFE91E63, false);
        gui.drawString(font, "PANEL DE CONTROL",
                (int)(x / scale) + font.width("INICIO") + 4, (int)(22 / scale), 0xFF000000, false);
        gui.pose().popMatrix();
        ...
        if (isWifi) {
            ...
            String pingTxt  = currentPingMs == -1 ? "Midiendo..."
                    : currentPingMs >= 999 ? "Sin respuesta"
                    : currentPingMs + " ms";
    ...
    /** Formatea segundos en "Xs", "Xm Ys" o "Xh Ym". */
